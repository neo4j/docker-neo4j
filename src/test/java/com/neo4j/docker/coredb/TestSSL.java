...
public class TestSSL {
    ...
    void testOpenSSLIsInstalledWithFIPS() throws Exception {
        ...
        Assertions.assertEquals(0, whichOpenSSL.getExitCode(), "openssl not installed! Full output:\n" + whichOpenSSL);
        Assertions.assertTrue(
                whichOpenSSL.getStdout().startsWith(OPENSSL_INSTALL_DIR),
        ...
        Assertions.assertTrue(
                openssl.get(0).contains("OpenSSL " + OPENSSL_VERSION),
                "OpenSSL " + OPENSSL_VERSION + " is not installed.\n" + versionOut);
        Assertions.assertTrue(
                openssl.stream().anyMatch(s -> s.matches("OPENSSLDIR:\s*\"" + OPENSSL_INSTALL_DIR + "\"?\\s*")),
                "OpenSSL is not using the expected ssl config directory:\n" + versionOut);
        Assertions.assertTrue(
                openssl.stream()
                        .anyMatch(s ->
                                s.matches("MODULESDIR:\s*\"" + OPENSSL_INSTALL_DIR + "/lib(64)?/ossl-modules\"?\\s*")),
        ...
        List<String> providers = Arrays.stream(providersOut.getStdout().split("\n"))
                ...
                .toList();
        Assertions.assertTrue(
                providers.stream().anyMatch(s -> s.matches("fips")),
        ...
        for (int i = fipsIdx; i < fipsIdx + 3; i++) {
            ...
            if (line.startsWith("name")) {
                Assertions.assertTrue(
                        line.matches("name:\\s+OpenSSL FIPS Provider"),
                        "FIPS provider has an unexpected name\n" + providersOut);
            } else if (line.startsWith("version")) {
                Assertions.assertTrue(
                        line.matches("version:\\s+" + OPENSSL_VERSION.replace(".", "\\.")),
                        "FIPS version is not " + OPENSSL_VERSION + ":\n" + providersOut);
            } else if (line.startsWith("status")) {
                Assertions.assertTrue(
                        line.matches("status:\\s+active"), "FIPS is not the active provider:\n" + providersOut);
            }
        }
    }
    ...
    void testFIPSOpenSSLLibrariesUsed() throws Exception {
        ...
        try (GenericContainer container = createContainer()) {
            ...
            container.start();
            String neo4jPID = container
                    .execInContainer("cat", "/var/lib/neo4j/run/neo4j.pid")
                    .getStdout();
            filesInUse = container
                    .execInContainer("su-exec", "neo4j", "cat", "/proc/" + neo4jPID + "/maps")
                    .getStdout();
        }
    ...
    private void verifyProcessAccessesFile(String filesInUse, String filename, String expectedPath) {
        ...
        Assertions.assertFalse(
                fileReads.isEmpty(),
                "Neo4j did not use " + filename + " at all." + " Actual files read were:\n" + filesInUse);
    ...
    void shouldFailIfDebianAndFIPS() throws Exception {
        Assumptions.assumeTrue(TestSettings.BASE_OS == BaseOS.BULLSEYE, "Test only applies to debian based images");
        try (GenericContainer container = createContainer()) {
            container.withEnv(FIPS_FLAG, "true");
            ...
            Assertions.assertTrue(
                    logs.contains("OpenSSL FIPS compatibility is only available in the Red Hat UBI9 Neo4j image"),
    ...
    void testEndToEndSSLEncryption() throws Exception {
        Assumptions.assumeFalse(
                TestSettings.BASE_OS == BaseOS.UBI9
    ...
    private synchronized Path getTCNativeBoringSSL() throws Exception {
        if (tcnativeBoringSSLJar == null) {
            ...
            try (GenericContainer container = HelperContainers.nginx()) {
                ...
                arch = arch.replace(
                        "aarch64", "aarch_64"); // for some reason netty arm libs use aarch_64 instead of aarch64
                String url = String.format(
                        "https://repo1.maven.org/maven2/io/netty/netty-tcnative-boringssl-static/"
                                + "%s/netty-tcnative-boringssl-static-%s-linux-%s.jar",
                        NETTY_TCNATIVE_VERSION, NETTY_TCNATIVE_VERSION, arch);
                ...
                Assertions.assertTrue(
                        tcnativeBoringSSLJar.toFile().exists(), "Could not download TCNative BoringSSL jar");
            }
        }
    ...
    void verifyEndToEndSSLEncryption(GenericContainer container) throws Exception {
        ...
        DatabaseIO dbio = new DatabaseIO(
                ...
                Config.builder()
                        .withEncryption()
                        .withTrustStrategy(Config.TrustStrategy.trustCustomCertificateSignedBy(certificates
                                .resolve(SSLCertificateFactory.CLIENT_CERTIFICATE_FILENAME)
                                .toFile()))
                        .build());
