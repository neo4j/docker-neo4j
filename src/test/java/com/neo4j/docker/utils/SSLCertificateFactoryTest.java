...
public class SSLCertificateFactoryTest {
    ...
    void generatesUnencryptedCertificateAndKey() throws Exception {
        ...
        File clientCert = outdir.resolve(SSLCertificateFactory.CLIENT_CERTIFICATE_FILENAME)
                .toFile();
    ...
    void shouldCreateUnencryptedKeyByDefault() throws Exception {
        Path outdir = folderManager.createFolder("certificates");
        new SSLCertificateFactory(outdir).withOwnerNeo4j().build();
        verifyCertificatesAndKey(outdir, SSLCertificateFactory.DEFAULT_HOST_NAME, null);
    }
    ...
    private void verifyFileOwnership(Path certificateDir, int expectedUID) throws Exception {
        ...
        Assertions.assertEquals(
                expectedUID,
                Files.getAttribute(cert, "unix:uid"),
                "Owner of certificate was not set to " + expectedUID);
        Assertions.assertEquals(
                CURRENT_USER_ID,
                Files.getAttribute(clientCert, "unix:uid"),
                "Owner of client certificate was not set to " + CURRENT_USER_ID);
        Assertions.assertEquals(
                expectedUID, Files.getAttribute(key, "unix:uid"), "Owner of private key was not set to " + expectedUID);
        if (passphrase.toFile().exists()) {
            Assertions.assertEquals(
                    expectedUID,
                    Files.getAttribute(passphrase, "unix:uid"),
    ...
    private void verifyCertificatesAndKey(Path certificateDir, String expectedHostName, @Nullable String keyPassphrase)
        ...
        try (GenericContainer container = HelperContainers.nginx()) {
            ...
            Container.ExecResult certModulus = container.execInContainer(
                    "openssl",
                    "x509",
                    "-in",
                    "/certificates/" + SSLCertificateFactory.CERTIFICATE_FILENAME,
                    "-inform",
                    "pem",
                    "-noout",
                    "-modulus");
            Container.ExecResult certSAN = container.execInContainer(
                    "sh",
                    "-c",
                    "openssl x509 -text -noout -in /certificates/" + SSLCertificateFactory.CERTIFICATE_FILENAME
                            + " | grep \"Subject Alternative Name\" -A1");
            Container.ExecResult clientCertModulus = container.execInContainer(
                    "openssl",
                    "x509",
                    "-in",
                    "/certificates/" + SSLCertificateFactory.CLIENT_CERTIFICATE_FILENAME,
                    "-inform",
                    "pem",
                    "-noout",
                    "-modulus");
            ...
            if (keyPassphrase == null) {
                keyModulus = container.execInContainer(
                        "openssl",
                        "rsa",
                        "-in",
                        "/certificates/" + SSLCertificateFactory.PRIVATE_KEY_FILENAME,
                        "-inform",
                        "pem",
                        "-noout",
                        "-modulus");
                ...
                Container.ExecResult keyRead = container.execInContainer(
                        "openssl",
                        "rsa",
                        "-in",
                        "/certificates/" + SSLCertificateFactory.PRIVATE_KEY_FILENAME,
                ...
                Assertions.assertNotEquals(
                        0, keyRead.getExitCode(), "Should have failed to read private key without passphrase.");
                // now read the key
                keyModulus = container.execInContainer(
                        "openssl",
                        "rsa",
            ...
            Assertions.assertEquals(
                    0, certModulus.getExitCode(), "Certificate was not created in x509 PEM format.\n" + certModulus);
            Assertions.assertEquals(
                    0,
                    clientCertModulus.getExitCode(),
                    "Client certificate was not created in x509 PEM format.\n" + clientCertModulus);
            Assertions.assertEquals(
                    0, keyModulus.getExitCode(), "Private key was not created in PEM format.\n" + keyModulus);
            Assertions.assertEquals(
                    certModulus.getStdout(), keyModulus.getStdout(), "Certificate and private key do not match");
            Assertions.assertEquals(
                    clientCertModulus.getStdout(),
                    keyModulus.getStdout(),
            ...
            Assertions.assertTrue(
                    certSAN.getStdout().contains(expectedHostName),
